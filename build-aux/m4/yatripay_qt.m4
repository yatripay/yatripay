...
dnl Output: If qt version is auto, set yatripay_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$yatripay_qt_want_version" = xauto && test "x$yatripay_qt_force" != xyes; then
    if test "x$yatripay_enable_qt" != xno; then
      AC_MSG_WARN([$1; yatripay-qt frontend will not be built])
    fi
    yatripay_enable_qt=no
    yatripay_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$yatripay_enable_qt" != xno && test "x$yatripay_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build yatripay-qt GUI (default=auto)])],
    [
     yatripay_qt_want_version=$withval
     if test "x$yatripay_qt_want_version" = xyes; then
       yatripay_qt_force=yes
       yatripay_qt_want_version=auto
     fi
    ],
    [yatripay_qt_want_version=auto])
...
dnl Outputs: yatripay_enable_qt, yatripay_enable_qt_dbus, yatripay_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$yatripay_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    yatripay_enable_qt=yes
    yatripay_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      yatripay_enable_qt_test=no
    fi
    yatripay_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      yatripay_enable_qt_dbus=yes
    fi
  ...
  if test x$yatripay_enable_qt = xyes; then
    AC_MSG_RESULT([$yatripay_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$yatripay_enable_qt])
  fi
...
dnl Output: yatripay_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, yatripay_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [yatripay_cv_static_qt=yes],
      [yatripay_cv_static_qt=no])
    ])
])
