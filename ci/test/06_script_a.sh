...
export P_CI_DIR="${BASE_BUILD_DIR}/yatripay-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/yatripay-config.h > src/config/yatripay-config.h.tmp && mv src/config/yatripay-config.h.tmp src/config/yatripay-config.h'
fi
