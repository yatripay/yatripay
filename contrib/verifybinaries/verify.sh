...
###   yatripaycore.org and yatripay.org and compares them.
...
WORKINGDIR="/tmp/yatripay_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://yatripaycore.org"
HOST2="https://yatripay.org"
BASEDIR="/bin/"
VERSIONPREFIX="yatripay-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'yatripay-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "yatripay.org failed to provide signature file, but yatripaycore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "yatripay.org and yatripaycore.org signature files were not equal?"
