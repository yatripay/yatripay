Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://yatripaycore.org/bin/yatripay-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/yatripay/yatripay/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `yatripayd -help` output for these two options for
...
- The `testnet` field in `yatripay-cli -getinfo` has been renamed to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Yatripay address without
...
- #15639 yatripay-wallet tool: Drop libyatripay_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in yatripayd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "yatripay-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of yatripayd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from yatripay_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_yatripay to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_yatripay output (practicalswift)
...
- #15896 feature_filelock, interface_yatripay_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_yatripay for expected errors (gertjaap)
...
- #16489 log: harmonize yatripayd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (yatripayVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example yatripay.conf (fanquake)
...
- #16080 build/doc: update yatripay_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in yatripay.conf (fanquake)
...
- #16621 add default yatripay.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the yatripay-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, yatripay.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/yatripayd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/yatripay/yatripay/).
