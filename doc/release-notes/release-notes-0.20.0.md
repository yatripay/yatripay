...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://yatripaycore.org/bin/yatripay-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/yatripay/yatripay/issues/16599),
  [PR #16702](https://github.com/yatripay/yatripay/pull/16702), and the
  `yatripayd help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://yatripaycore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/yatripay-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.yatripaycore.org.  (#17596)
...
- The `yatripay-cli` utility used with the `-getinfo` parameter now
...
- The `yatripay-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `yatripaystrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_yatripay-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_yatripay` binary (hebasto)
...
- #18702 Fix ASLR for yatripay-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate yatripay schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest yatripay.conf (sdaftuar)
...
- #18022 Fix appveyor `test_yatripay` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (yatripay-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone yatripay-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken yatripay-cli examples (andrewtoth)
...
- #17503 Remove bitness from yatripay-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.yatripaycore.org (laanwj)
...
- #17648 Rename wallet-tool references to yatripay-wallet (hel-o)
...
- #16115 On yatripayd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from yatripay-cli -getinfo (malevolent)
