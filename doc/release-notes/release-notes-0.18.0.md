Bitcoin Core version 0.18.0 is now available from:
  ...
  <https://yatripaycore.org/bin/yatripay-core-0.18.0/>
...
`/Applications/Bitcoin-Qt` (on Mac) or `yatripayd`/`yatripay-qt` (on
Linux).
...
`contrib/init/yatripayd.service`, it has been changed to use
`/var/lib/yatripayd` as the data directory instead of
`~yatripay/.yatripay`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/yatripayd` will
exist has enough space (check using `df -h /var/lib/yatripayd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/yatripay/yatripay/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://yatripaycore.org/en/contact/) the Bitcoin Core
...
The systemd init file (`contrib/init/yatripayd.service`) has been changed
to use `/var/lib/yatripayd` as the data directory instead of
`~yatripay/.yatripay`. This change makes Bitcoin Core more consistent with
...
`datadir` option in `/etc/yatripay/yatripay.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/yatripay/yatripay.conf`.


Documentation
-------------

- A new short [document](https://github.com/yatripay/yatripay/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/yatripay/yatripay/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/yatripay/yatripay/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/yatripay/yatripay/blob/master/doc/yatripay-conf.md)
  about the `yatripay.conf` file describes how to use it to configure
  Bitcoin Core.

- A new document introduces Bitcoin Core's BIP174 [Partially-Signed
  Bitcoin Transactions
  (PSBT)](https://github.com/yatripay/yatripay/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/yatripay/yatripay/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/yatripay/yatripay/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/yatripay/yatripay/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/yatripay/yatripay/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/yatripay/yatripay/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart yatripayd with the `-deprecatedrpc=generate`
...
- A new `yatripay-wallet` tool is now distributed alongside Bitcoin
...
- A sub-project of Bitcoin Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/yatripay-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `yatripay_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libyatripay_server.a` (jnewbery)
...
- #13926 [Tools] yatripay-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like yatripayd (MarcoFalke)
...
- #14521 qt, docs: Fix `yatripay-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libyatripay_server` (laanwj)
...
- #15154 Configure: yatripay-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/yatripay-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libyatripay_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in yatripay-util-test.py (isghe)
...
- #12246 Bugfix: Only run yatripay-tx tests when yatripay-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_yatripay` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_yatripay` (MarcoFalke)
...
- #14186 yatripay-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_yatripay.conf` message on startup if conf file exists (leishman)
...
- #12255 Update yatripay.service to conform to init.md (dongcarl)
...
- #14497 Add doc/yatripay-conf.md (hebasto)
...
- #15354 Add missing `yatripay-wallet` tool manpages (MarcoFalke)
...
- #15765 correct yatripayconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support yatripay folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/yatripay/).
