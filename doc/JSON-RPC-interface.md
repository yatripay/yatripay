...
The headless daemon `yatripayd` has the JSON-RPC API enabled by default, the GUI
`yatripay-qt` has it disabled by default. This can be changed with the `-server`
...
- **Securing remote network access:** You may optionally allow other
  ...
  `yatripayd -help` for more information about these settings and other
