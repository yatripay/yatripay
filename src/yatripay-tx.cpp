...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " yatripay-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  yatripay-tx [options] <hex-tx> [commands]  Update hex-encoded yatripay transaction\n" +
            "or:     yatripay-tx [options] -create [commands]   Create hex-encoded yatripay transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded yatripay transaction
