...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. yatripay_de.qm (shortcut "de" needs to be defined in yatripay.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. yatripay_de_DE.qm (shortcut "de_DE" needs to be defined in yatripay.qrc)
...
static const char* qt_argv = "yatripay-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        // yatripay: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(yatripay);
    Q_INIT_RESOURCE(yatripay_locale);
    ...
    /// 6. Determine availability of data directory and parse yatripay.conf
    ...
    // yatripay: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
